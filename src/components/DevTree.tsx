...
export const DevTree = ({ data }: DevTreeProps) => {
  const dataLinks = JSON.parse(data.links).filter(
    (item: SocialNetwork) => item.enabled
  );
  const [enabledLinks, setEnabledLinks] = useState<SocialNetwork[]>(dataLinks);

  useEffect(() => {
    setEnabledLinks(dataLinks);
  }, [data]);
        ...
        <main className="mx-auto max-w-5xl p-10 md:p-0">
          ...
          <div className="flex flex-col md:flex-row gap-10 mt-10">
            ...
            <div className="w-full md:w-96 bg-slate-800 px-5 py-10 space-y-6">
              ...
              >
                <div className="mt-20 flex flex-col gap-5 relative overflow-hidden">
                  <SortableContext
                    items={enabledLinks}
